...
#include "local_dependence_graph.h"
...
bool local_dep_graph_domaint::merge(
...
void local_dep_graph_domaint::control_dependencies(
  goto_programt::const_targett from,
  goto_programt::const_targett to,
  local_dependence_grapht &dep_graph)
{
  ...
  for(depst::const_iterator
      ...
      it!=control_deps.end();
      ++it)
    dep_graph.add_dep(local_dep_edget::CTRL, *it, to);
}
...
void local_dep_graph_domaint::data_dependencies(
  goto_programt::const_targett from,
  goto_programt::const_targett to,
  local_dependence_grapht &dep_graph,
  const namespacet &ns)
{
  ...
  forall_rw_range_set_r_objects(it, rw_set)
  {
    ...
    const local_rd_range_domaint::ranges_at_loct &w_ranges=
      dep_graph.reaching_definitions()[to].get(it->first);

    for(local_rd_range_domaint::ranges_at_loct::const_iterator
        w_itl=w_ranges.begin();
        w_itl!=w_ranges.end();
        ++w_itl)
    {
      bool found=false;
      for(local_rd_range_domaint::rangest::const_iterator
      ...
      ++it)
  {
    ...
    dep_graph.add_dep(local_dep_edget::DATA, *it, to);
  }
}
...
void local_dep_graph_domaint::transform(
  goto_programt::const_targett from,
  goto_programt::const_targett to,
  local_ai_baset &ai,
  const namespacet &ns)
{
  local_dependence_grapht *dep_graph=dynamic_cast<local_dependence_grapht*>(&ai);
  assert(dep_graph!=0);
  ...
  if(from->is_function_call())
  {
    ...
    local_dep_graph_domaint *s=
      dynamic_cast<local_dep_graph_domaint*>(&(dep_graph->get_state(next)));
    assert(s!=0);
...
void local_dep_graph_domaint::output(
  std::ostream &out,
...
void local_dependence_grapht::add_dep(
  local_dep_edget::kindt kind,
  goto_programt::const_targett from,
  goto_programt::const_targett to)
{
  const unsigned n_from=func_states_map[from->function][from].get_node_id();
  assert(n_from<size());
  const unsigned n_to=func_states_map[to->function][to].get_node_id();
  assert(n_to<size());
